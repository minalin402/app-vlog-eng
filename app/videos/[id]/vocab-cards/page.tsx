...
import { ChevronLeft, Volume2, Bookmark, Eye, EyeOff, Loader2, Filter } from "lucide-react"
...
export default function VocabCardsPage() {
  ...
  const [onlyFav, setOnlyFav] = useState(false) // 仅看收藏开关
  ...
  useEffect(() => {
    ...
    async function loadRealData() {
      try {
        const { data: vocabData } = await supabase.from('vocabulary_items').select('*').eq('video_id', videoId)
        const vocabularies = (vocabData || []).filter(v => v.type === 'word').map(v => ({
          ...
          chinese_definition: v.definition_zh, example_from_video: v.example_en, example_translation: v.example_zh,
        }))
        ...
        setVideoData({ vocabularies, phrases, expressions })

        const { data: { user } } = await supabase.auth.getUser()
        if (user) {
          const { data: favData } = await supabase.from('user_favorites').select('vocabulary_id').eq('user_id', user.id).not('vocabulary_id', 'is', null)
          if (favData) {
            const initialFavs: Record<string, boolean> = {}
            favData.forEach(f => { initialFavs[f.vocabulary_id] = true })
  ...
  const handleToggleFavorite = async (id: string, type: "word" | "phrase" | "expression") => {
    ...
    try { await toggleFavoriteAPI(id, type, targetState) } 
    catch (error) { setFavState((prev) => ({ ...prev, [id]: isFav })) }
  }
  ...
  // 根据当前 Tab 获取列表
  let currentList = activeTab === "word" ? words : activeTab === "phrase" ? phrases : expressions
  // 根据收藏开关过滤
  if (onlyFav) {
    currentList = currentList.filter((item: any) => favState[item.id])
  }
...
// ─── 卡片组件 (直接复刻 Vocab Panel 的样式，加上白底和阴影) ─────────────────────────
